...
NES_sdl: $(BUILDDIR)/cpu6502.sig $(BUILDDIR)/cpu6502.sml $(SMLDIR)/run.sml $(CDIR)/nes-ppu-draw-sdl.c $(CDIR)/nes-ppu.c $(CDIR)/nes-mem.c
	mlton -verbose $(VERBOSE) -output $@ -default-ann 'allowFFI true' -export-header $(BUILDDIR)/smlexport.h -cc-opt "-I $(BUILDDIR)/" -link-opt "-lpthread -lSDL" -default-type intinf $(SMLDIR)/cpu6502.mlb $(CDIR)/nes-ppu-draw-sdl.c $(CDIR)/nes-ppu.c $(CDIR)/nes-mem.c
